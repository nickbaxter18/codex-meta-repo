...
class CustomerProfile(TimestampMixin, SQLModel, table=True):
...
class EquipmentMedia(TimestampMixin, SQLModel, table=True):
...
class Reservation(TimestampMixin, SQLModel, table=True):
...
class MaintenanceLog(TimestampMixin, SQLModel, table=True):
